...
public class InnerPackDecrypter {
    ...
    public static HashMap<String, byte[]> decryptPack(InputStream packStream, String grabKey, String confirmKey) {
        int dummy = 0;
        HashMap<String, byte[]> classes = new HashMap<String, byte[]>();
        try {
            byte[] key = decrypt(grabKey);
            byte[] ivParameterSpec = decrypt(confirmKey);
            Cipher cipherObject;
            ...
            int bytesread;
            try {
                int bytesRead = packStream.read(buffer, offset, 5242880 - offset);

                while (-1 != bytesRead) {
                    offset += bytesRead;
                    bytesRead = packStream.read(buffer, offset, -offset + 5242880);
                    bytesread = dummy;
            ...
            label191:
            {
                try {
                    ...
                    while ((entry = jarInputStream.getNextJarEntry()) != null) {
                        ...
                        offset = 0;
                        bytesread = jarInputStream.read(buffer, offset, 5242880 + -offset);
                        if (dummy != 0) {
                            break label191;
                        }
                        ...
                        label143:
                        {
                            while (true) {
                                if (0 != ~bytesRead1) {
                                    offset += bytesRead1;
                                    bytesRead1 = jarInputStream.read(buffer, offset, -offset + 5242880);
                                    bytesread = dummy;
                            ...
                            classes.put(entryName.substring(0, entryName.length() + -6), desBytes);
                        }
